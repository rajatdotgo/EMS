...
public class EmployeeService {
    ...
    public ResponseEntity deleteEmployee(int id) {
        if (!empValidate.empExist(id)) {
            ...
            if (emp.getDesgName().equals("Director")) {
                if (!empRepo.findAllByParentId(emp.getEmpId()).isEmpty()) {
    ...
    public ResponseEntity employeeUpdate(int oldId, PutRequest emp) {
        ...
        if (emp.getJobTitle() != null &&( !emp.getJobTitle().equals(""))) {
            if (empRepo.findByEmpId(oldId).designation.getDesId() == 1 && (!emp.getJobTitle().equals("Director"))) {
                return new ResponseEntity("You can not alter designation of  Director ", HttpStatus.BAD_REQUEST);
            }
            if (emp.getManagerId() == null) {
    ...
    public ResponseEntity replaceEmployee(int empId, PutRequest emp) {
        if (!empValidate.desExist(emp.getJobTitle()))
            return new ResponseEntity("Designation does not exist please enter a valid one", HttpStatus.BAD_REQUEST);
        if (emp.getName() == null || emp.getName().trim().equals(""))
            return new ResponseEntity("Please enter the name of the employee", HttpStatus.BAD_REQUEST);
